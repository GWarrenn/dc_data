...
ggplot(closest_metro,aes(x=as.numeric(closest_metro$distance),y=closest_metro$adj_price)) + geom_point() + facet_wrap( ~ quadrant)
...
ggmap(map) + stat_summary_2d(geom = "tile",bins = 50,data=closest_metro, aes(x = lon, y = lat, z = closest_metro$walking_time), alpha=0.5) +
...
ggmap(map) + stat_summary_2d(geom = "tile",bins = 50,data=closest_metro_trim, aes(x = lon, y = lat, z = closest_metro_trim$walking_time), alpha=0.5) +
  scale_fill_gradient(low = "yellow", high = "red", guide = guide_legend(title = "Seconds")) +xlab("") + ylab("") +
  theme(axis.title=element_blank(),
        axis.text=element_blank(),
        axis.ticks=element_blank()) + ggtitle("Walking Distance to Metro")
...
qmplot(lon,lat,data=closest_metro,maptype = "toner-lite", color = NAME) + scale_colour_discrete(guide=FALSE) + 
    stat_summary_2d(geom = "tile",bins = 50,data=closest_metro, aes(x = lon, y = lat, z = closest_metro$walking_time), alpha=0.5)
...
ggplot(avg_price_by_distance, aes(x = reorder(avg_price_by_distance$NAME, avg_price_by_distance$average_price), 
...
ggplot(avg_price_by_time, aes(x = reorder(avg_price_by_time$NAME, avg_price_by_time$average_price), 
...
##other analysis
  ...
  #average home price by metro stop
    #--most/least expensive metro to live within 100 feet of?
  #number of lines?